...
- **용도:** 사용자가 바로 인지하고 다음 동작으로 넘어가야 하는 안내/오류 상태(예: 단어 추출 실패).
- **레이아웃:** 제목(필수) → 보조 설명(선택) → 하단 단일 CTA 버튼.
- **버튼 규칙:** 하단 버튼은 full-width Primary 1개만 사용하고, 라벨은 다음 행동을 직접 표현(`다시 시도하기`).
- **카피 규칙:** 제목은 상태를 단정형으로 명확히 전달, 설명은 원인/해결 방향을 1문장으로 짧게 안내.
- **간격 가이드:** 모달 좌우 16dp 여백, 제목-설명 8dp, 설명-버튼 20dp.
- **Dismiss 규칙:** 바깥 탭 dismiss 허용 가능하나, 기본은 CTA로 닫히는 흐름을 우선한다.
- **참고 목업:** `안드로이드 360_800 단어 추출 모달.png`
